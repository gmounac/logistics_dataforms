...
def test_visit_count_increments_each_gate_in(yard):
    ...
    yard.gate_out(
        CONT_A, at=NOW - timedelta(days=4), hauler=Hauler.HD,
        destination=Destination.LML, cargo_status=ContainerStatus.EMPTY,
    )
...
def test_dry_container_pti_status_rules(yard):
    with pytest.raises(YardError, match="only applies to reefers"):
        yard.gate_in(
            dry(CONT_A), at=NOW - timedelta(days=1), hauler=Hauler.HD,
            cargo_status=ContainerStatus.FULL, pti_status=PTIStatus.PTI,
        )
...
def test_reefer_gate_in_needs_pti_status(yard):
    with pytest.raises(YardError, match="pti_status is required"):
        yard.gate_in(
            reefer(CONT_A), at=NOW - timedelta(days=1), hauler=Hauler.HD,
            cargo_status=ContainerStatus.FULL, pti_status=None,
        )
...
def test_plug_in_rejects_set_point_below_unit_minimum(yard):
    ...
    with pytest.raises(YardError, match="below the"):
        yard.plug_in(
            CONT_A, at=NOW - timedelta(hours=1), purpose=PlugPurpose.STORAGE,
            set_point_c=-40, seal_number="L0059326",
        )
...
def test_double_plug_in_is_rejected(yard):
    ...
    with pytest.raises(YardError, match="already plugged in"):
        yard.plug_in(
            CONT_A, at=NOW - timedelta(hours=9), purpose=PlugPurpose.PTI,
            generator=Generator.K7, set_point_c=-18,
        )
...
def test_gate_out_blocked_while_plugged_in(yard):
    ...
    with pytest.raises(YardError, match="still plugged in"):
        yard.gate_out(
            CONT_A, at=NOW - timedelta(hours=1), hauler=Hauler.HD,
            destination=Destination.LML, cargo_status=ContainerStatus.FULL,
        )
...
def test_one_clean_per_visit_unless_cross_stuffed(yard):
    ...
    st = yard.clean(
        CONT_A, at=NOW - timedelta(hours=7), result=CleaningResult.CLEAN,
        cross_stuffed=True,
    )
...
def test_cleaning_resets_after_gate_out_and_back_in(yard):
    gate_in_reefer(yard, at=NOW - timedelta(days=5))
    yard.clean(CONT_A, at=NOW - timedelta(days=5) + timedelta(hours=1), result=CleaningResult.CLEAN)
    yard.gate_out(
        CONT_A, at=NOW - timedelta(days=4), hauler=Hauler.HD,
        destination=Destination.LML, cargo_status=ContainerStatus.EMPTY,
    )
...
def _two_on_site(yard):
    ...
    yard.gate_in(
        reefer(CONT_B), at=NOW - timedelta(days=1), hauler=Hauler.HD,
        cargo_status=ContainerStatus.EMPTY, pti_status=PTIStatus.NON_PTI,
    )
...
def test_cross_stuff_into_container_moves_cargo_state(yard):
    _two_on_site(yard)
    yard.cross_stuff(
        CONT_A, at=NOW - timedelta(hours=3), ended_at=NOW - timedelta(hours=2),
        target=CrossStuffTarget.CONTAINER, new_container_number=CONT_B,
...
def test_cross_stuff_to_cold_storage_needs_no_new_container(yard):
    ...
    st = yard.cross_stuff(
        CONT_A, at=NOW - timedelta(hours=3), ended_at=NOW - timedelta(hours=2),
        target=CrossStuffTarget.COLD_STORAGE, original_emptied=True,
    )
...
def test_cross_stuff_receiver_must_be_on_site(yard):
    ...
    with pytest.raises(YardError, match="not in the yard"):
        yard.cross_stuff(
            CONT_A, at=NOW - timedelta(hours=3), ended_at=NOW - timedelta(hours=2),
            target=CrossStuffTarget.CONTAINER, new_container_number=CONT_C,
        )
...
def test_cross_stuff_into_itself_is_rejected(yard):
    ...
    with pytest.raises(YardError, match="same as the original"):
        yard.cross_stuff(
            CONT_A, at=NOW - timedelta(hours=3), ended_at=NOW - timedelta(hours=2),
            target=CrossStuffTarget.CONTAINER, new_container_number=CONT_A,
        )
...
def _plug_for_temps(yard):
    gate_in_reefer(yard)
    yard.plug_in(
        CONT_A, at=NOW - timedelta(hours=12), purpose=PlugPurpose.STORAGE,
        set_point_c=-18, seal_number="L0059326", cargo_status=ContainerStatus.FULL,
    )
...
def test_temperature_check_requires_plugged_in(yard):
    ...
    with pytest.raises(YardError, match="not plugged in"):
        yard.temperature_check(
            CONT_A, at=NOW, time_slot=TimeSlot.AM, set_point_c=-18,
            supply_temp_c=-17, return_temp_c=-16, remark=TemperatureRemark.IN_RANGE,
        )
...
def test_temperature_reading_before_plug_in_is_rejected(yard):
    ...
    with pytest.raises(YardError, match="before the plug in"):
        yard.temperature_check(
            CONT_A, at=NOW - timedelta(hours=20), time_slot=TimeSlot.AM,
            set_point_c=-18, supply_temp_c=-17, return_temp_c=-16,
...
def test_temperature_readings_out_of_order_are_allowed(yard):
    _plug_for_temps(yard)
    yard.temperature_check(
        CONT_A, at=NOW - timedelta(hours=4), time_slot=TimeSlot.PM,
        set_point_c=-18, supply_temp_c=-17, return_temp_c=-16,
        remark=TemperatureRemark.IN_RANGE,
    )
    # earlier slot, entered afterwards
    yard.temperature_check(
        CONT_A, at=NOW - timedelta(hours=8), time_slot=TimeSlot.NOON,
        set_point_c=-18, supply_temp_c=-17, return_temp_c=-16,
        remark=TemperatureRemark.IN_RANGE,
    )
